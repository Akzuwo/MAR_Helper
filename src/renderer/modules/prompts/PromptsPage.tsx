...
export function PromptsPage() {
  ...
  ].sort((a, b) => b.createdAt.localeCompare(a.createdAt)), [filteredChats, filteredStandalone]);
  ...
  const navigateView = useCallback((navigation: () => void, direction: 'forward' | 'back') => {
  ...
  useEffect(() => {
    if (viewPhase !== 'exit') return;
    const fallback = window.setTimeout(finishViewTransition, 240);
    return () => window.clearTimeout(fallback);
  }, [finishViewTransition, viewPhase]);
  ...
  const save = (entry: PromptEntry) => {
    ...
    if (!selectedId) navigateView(() => setSelectedId(entry.id), 'forward');
  };
  ...
  const saveChat = (title: string) => {
    if (editingChat) {
      ...
      let createdId = '';
      void updateState((current) => { const result = createPromptChat(current, title); createdId = result.chat.id; return result.state; }, 'Chat erstellt');
      navigateView(() => setSelectedChatId(createdId), 'forward');
    }
  ...
  const confirmDeletePrompt = () => {
    ...
    setDeleteEntry(null);
    navigateView(() => {
      void updateState((current) => ({ ...current, promptEntries: current.promptEntries.filter((entry) => entry.id !== entryId) }), 'Prompt gelöscht');
      setSelectedId(null);
    }, 'back');
  };
  ...
  const confirmDeleteChat = () => {
    ...
    navigateView(() => {
      void updateState((current) => ({
        ...current,
        promptChats: current.promptChats.filter((chat) => chat.id !== chatId),
        promptEntries: current.promptEntries.filter((entry) => entry.chatId !== chatId)
      }), 'Chat gelöscht');
      setSelectedId(null);
  ...
  if (selected) pageContent = <PromptDetail entry={selected} chats={state.promptChats} onBack={() => navigateView(() => setSelectedId(null), 'back')} onEdit={() => { setEditing(selected); setEditorOpen(true); }} onDelete={() => setDeleteEntry(selected)} onAssign={!selected.chatId && state.promptChats.length ? () => { setAssignChatId(state.promptChats[0].id); setAssignOpen(true); } : undefined} onCopied={() => toast('Prompt und Antwort kopiert')} onRemoveGit={() => void updateState((current) => ({ ...current, promptEntries: current.promptEntries.map((entry) => entry.id === selected.id ? { ...entry, gitSnapshot: undefined, updatedAt: new Date().toISOString() } : entry) }), 'Git-Verknüpfung entfernt')}/>;
    ...
    {chatEntries.length === 0 ? <EmptyState icon={<MessagesSquare/>} title="Dieser Chat ist noch leer" description="Erfasse den ersten Prompt. Er erhält automatisch die Nummer des Chats mit dem Zusatz .1." action={<Button icon={<Plus size={17}/>} onClick={() => setEditorOpen(true)}>Prompt hinzufügen</Button>}/> :
      <div className="prompt-grid" role="list">{chatEntries.map((entry) => <button className="prompt-card" key={entry.id} role="listitem" onClick={() => navigateView(() => setSelectedId(entry.id), 'forward')}>
    ...
    {!hasContent ? <EmptyState icon={<WandSparkles/>} title="Noch keine Prompts" description="Erfasse einen einzelnen Prompt oder erstelle einen Chat für mehrere zusammengehörige Prompts." action={<Button icon={<Plus size={17}/>} onClick={() => setEditorOpen(true)}>Einzelprompt erfassen</Button>}/> : topLevel.length === 0 ? <EmptyState icon={<Search/>} title="Keine Treffer" description="Passe Suche oder Modellfilter an."/> :
      <div className="prompt-grid" role="list">{topLevel.map((item) => item.kind === 'chat' ? <button className="prompt-card prompt-card--chat" key={item.chat.id} role="listitem" onClick={() => navigateView(() => setSelectedChatId(item.chat.id), 'forward')}>
      ...
      </button> : <button className="prompt-card" key={item.entry.id} role="listitem" onClick={() => navigateView(() => setSelectedId(item.entry.id), 'forward')}>
  ...
  return <>
    <div
      className={`prompt-view prompt-view--${viewPhase} prompt-view--${viewDirection}`}
      onAnimationEnd={(event) => {
        if (event.target !== event.currentTarget) return;
        if (viewPhase === 'exit') finishViewTransition();
